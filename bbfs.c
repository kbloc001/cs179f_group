...
*/
/* As modified by Thomas Payne (thp) on 10/24/15.  See note at end of this file. */
#include "params.h"
#include <ctype.h>
#include <dirent.h>
#include <errno.h>
	     ...
	     struct fuse_file_info *fi)
{
    ...
    retstat = my_pwrite(fi->fh, buf, size, offset);
    if (retstat < 0)
...
int bb_releasedir(const char *path, struct fuse_file_info *fi)
{
    ...
    my_closedir((MY_DIR *) (uintptr_t) fi->fh);
...
/* Thomas Payne 10/24/2015 This a slightly modified version of version
  of Joseph Pfeiffer's bbfs.c.  I've placed the prefix "my_" onto all
  calls to the underlying Unix library, which I presume is glibc and
  "MY_" onto all occurences of the macro "DIR".  Also, I've added the
  line "#include my_stubs.H".

  I intend to substitute my own definitions of those functions and
  DIR, which I will place in the C++ files my_stubs.cc and my_stubs.H.

  This code makes one semantic shift: open(), close(), pread() and
  pwrite() work in terms of file handles, which are id numbers for
  inodes (a.k.a. "file handles"), rather than file descriptors, which
  are id numbers for entries in per-process open-file tables.  Note
  that Pfeiffer's code stores the value returned by open into the
  fh (file handle) field of a fuse_file_info struct.
*/
